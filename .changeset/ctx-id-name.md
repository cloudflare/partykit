...
- `getServerByName` no longer makes a `setName()` RPC when called without props — it returns the stub directly (0 RPCs). With props it remains 1 RPC, so `onStart(props)` receives them as before.
